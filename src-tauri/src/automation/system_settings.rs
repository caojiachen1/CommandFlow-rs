...
use windows::{
	...
	Win32::{
		...
		NetworkManagement::IpHelper::{
			FreeMibTable, GetIfEntry, GetIfTable2, SetIfEntry, MIB_IFROW, MIB_IF_TABLE2,
			IF_ADMIN_STATUS_DOWN, IF_ADMIN_STATUS_UP, IF_TYPE_SOFTWARE_LOOPBACK,
		},
		System::{
	...
	#[cfg(target_os = "windows")]
	{
		let join_result = tokio::task::spawn_blocking(move || {
			with_audio_endpoint("设置系统音量", |endpoint| unsafe {
				endpoint.SetMasterVolumeLevelScalar(safe_percent as f32 / 100.0, std::ptr::null())?;
				Ok(())
			})
		})
		.await
		.map_err(|error| CommandFlowError::Automation(format!("音量设置任务执行失败：{}", error)))?;
...
pub async fn adjust_volume(delta: i32) -> CommandResult<()> {
	#[cfg(target_os = "windows")]
	{
		...
		.await
		.map_err(|error| CommandFlowError::Automation(format!("音量调节任务执行失败：{}", error)))?;
...
pub async fn switch_wifi(state: &str) -> CommandResult<()> {
	...
	#[cfg(target_os = "windows")]
	{
		...
		.await
		.map_err(|error| CommandFlowError::Automation(format!("WiFi 切换任务执行失败：{}", error)))?;
...
pub async fn switch_bluetooth(state: &str) -> CommandResult<()> {
	...
	#[cfg(target_os = "windows")]
	{
		...
		.await
		.map_err(|error| CommandFlowError::Automation(format!("蓝牙切换任务执行失败：{}", error)))?;
...
pub async fn switch_network_adapter(adapter_name: Option<&str>, state: &str) -> CommandResult<()> {
	...
	#[cfg(target_os = "windows")]
	{
		...
		.await
		.map_err(|error| CommandFlowError::Automation(format!("网卡切换任务执行失败：{}", error)))?;
...
pub async fn set_theme(mode: &str) -> CommandResult<()> {
	...
	#[cfg(target_os = "windows")]
	{
		...
		let join_result = tokio::task::spawn_blocking(move || set_windows_theme(&normalized_clone))
			.await
			.map_err(|error| CommandFlowError::Automation(format!("主题切换任务执行失败：{}", error)))?;
		return join_result;
	}
...
pub async fn set_power_plan(plan: &str) -> CommandResult<()> {
	...
	{
		let normalized_clone = normalized.clone();
		let join_result = tokio::task::spawn_blocking(move || set_windows_power_plan(&normalized_clone))
			.await
			.map_err(|error| CommandFlowError::Automation(format!("电源计划切换任务执行失败：{}", error)))?;
		return join_result;
	}
...
pub async fn open_settings_page(page: &str) -> CommandResult<()> {
	...
	{
		let normalized_clone = normalized.clone();
		let join_result = tokio::task::spawn_blocking(move || open_windows_settings_page(&normalized_clone))
			.await
			.map_err(|error| CommandFlowError::Automation(format!("打开设置页任务执行失败：{}", error)))?;
		return join_result;
	}

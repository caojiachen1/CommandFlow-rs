...
mod windows_impl {
    ...
    fn collect_search_roots(
        ...
        for _ in 0..POINT_HIT_SEARCH_MAX_ANCESTOR_HOPS {
            let Some(parent) = unsafe { control_walker.GetParentElement(&current_control) }.ok()
            else {
                break;
            };
    ...
    fn with_automation<T>(
        run: impl FnOnce(&IUIAutomation) -> CommandResult<T>,
    ) -> CommandResult<T> {
    ...
    fn collect_descendants(
        ...
        let array = unsafe { root.FindAll(TreeScope_Subtree, &condition) }.map_err(|error| {
            CommandFlowError::Automation(format!("UIA FindAll 失败：{}", error))
        })?;
    ...
    fn sibling_index(
        ...
        let children =
            unsafe { parent.FindAll(TreeScope_Children, &condition) }.map_err(|error| {
                CommandFlowError::Automation(format!("UIA 查找子节点失败：{}", error))
            })?;
    ...
    fn fast_preview_matches_locator(
        locator: &UiElementLocator,
        preview: &UiElementPreview,
    ) -> bool {
        ...
        if let Some(expected) = normalize_option(&locator.parent_automation_id) {
            let actual =
                normalize_option(&preview.locator.parent_automation_id).unwrap_or_default();
            if actual != expected {
                return false;
            }
        }
...
#[cfg(target_os = "windows")]
pub use windows_impl::{inspect_element_at_point, resolve_locator, resolve_locator_center};
...
mod linux_stub {
    ...
    pub fn inspect_element_at_point(_x: i32, _y: i32) -> CommandResult<Option<UiElementPreview>> {
        Err(CommandFlowError::Automation(
            "UI 元素识别仅 supported on Windows.".to_string(),
        ))
    }

    pub fn resolve_locator_center(_locator: &UiElementLocator) -> CommandResult<(i32, i32)> {
        Err(CommandFlowError::Automation(
            "UI 元素识别仅 supported on Windows.".to_string(),
        ))
    }

    pub fn resolve_locator(_locator: &UiElementLocator) -> CommandResult<UiElementPreview> {
        Err(CommandFlowError::Automation(
            "UI 元素识别仅 supported on Windows.".to_string(),
        ))
    }
...
#[cfg(not(target_os = "windows"))]
pub use linux_stub::{inspect_element_at_point, resolve_locator, resolve_locator_center};

...
export default function SmartInputSelect({ value, placeholder, options, onChange, onOptionSelect, onEnter, hint, filterOptions = true }: SmartInputSelectProps) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <div className="cf-card cf-prop-dropdown-shell flex items-center gap-1 rounded-xl border border-slate-200 bg-white/85 px-2 py-1.5 backdrop-blur transition-all focus-within:border-cyan-500 focus-within:ring-4 focus-within:ring-cyan-500/10 dark:border-neutral-700 dark:bg-neutral-900/80">
        <input
          type="text"
          value={value}
      ...
      {open ? (
        <div
          ref={menuRef}
          className="cf-card cf-prop-dropdown-menu absolute z-[260] mt-1.5 max-h-52 w-full overflow-auto rounded-xl border border-slate-200 bg-white/95 p-1 backdrop-blur-md dark:border-neutral-700 dark:bg-neutral-900/95"
        >

...
export default function SmartInputSelect({ value, placeholder, options, onChange, hint }: SmartInputSelectProps) {
  ...
  useEffect(() => {
    if (!open) return
    const bounded = Math.max(0, Math.min(activeIndex, allOptions.length - 1))
    setActiveIndex(bounded)
  }, [activeIndex, allOptions.length, open])
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'ArrowDown') {
      ...
      setOpen(true)
      setActiveIndex((idx) => Math.min(idx + 1, allOptions.length - 1))
      return
    }

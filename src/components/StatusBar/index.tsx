...
export default function StatusBar() {
  ...
  const cursorModeLabel = cursor.isPhysicalPixel ? '全局物理坐标' : '全局屏幕坐标'

  return (
    <footer className="cf-window cf-pane relative z-50 flex h-10 items-center gap-6 border-t border-slate-200 bg-white px-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-slate-400">
      <div className="flex items-center gap-2">
        <span className="h-2 w-2 rounded-full bg-slate-300 dark:bg-neutral-700" />
        <span>全局坐标: <span className="font-mono text-cyan-600 dark:text-cyan-400">{cursor.x}, {cursor.y}</span></span>
        <span className="ml-1 opacity-50 text-[9px]">({cursorModeLabel})</span>
      </div>
      ...
      <div className="cf-card ml-auto flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-1 dark:bg-neutral-800/50">

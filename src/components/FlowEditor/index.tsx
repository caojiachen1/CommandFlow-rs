...
import { ALL_NODE_KINDS, getNodePaletteItem, NODE_PALETTE_CATEGORIES } from '../../utils/nodeCatalog'
...
function InnerFlowEditor({ onPaneClick }: { onPaneClick?: () => void }) {
  ...
  const nodeTypes = useMemo(
    () => ({
      ...
      lock: ClickNode,
      signOut: ClickNode,
      volumeMute: ClickNode,
      volumeSet: ClickNode,
      volumeAdjust: ClickNode,
      brightnessSet: ClickNode,
      wifiSwitch: ClickNode,
      bluetoothSwitch: ClickNode,
      networkAdapterSwitch: ClickNode,
      theme: ClickNode,
      powerPlan: ClickNode,
      openSettings: ClickNode,
  ...
  const quickInsertCategoryEntries = useMemo(() => {
    if (!quickInsert) return []

    const categoryOrder = NODE_PALETTE_CATEGORIES.map((c) => c.title)
    const grouped = new Map<string, typeof filteredQuickInsertItems>()

    for (const item of filteredQuickInsertItems) {
    ...
    return categoryOrder
      ...
      .map((title) => ({ category: title, items: grouped.get(title)! }))
  }, [quickInsert, filteredQuickInsertItems])
  ...
  return (
    ...
    >
      ...
      {quickInsert ? (
        <div
          className="cf-window cf-card absolute z-[150] w-[300px] rounded-xl border border-slate-200 bg-white/95 shadow-2xl backdrop-blur dark:border-neutral-700 dark:bg-neutral-900/95"
        ...
        >
          <div className="border-b border-slate-200 px-3 py-2 dark:border-neutral-800">
            <div className="relative">
              ...
              <input
                ...
                placeholder="搜索节点"
                className="h-8 w-full rounded-md border border-slate-300 bg-white/85 pl-8 pr-6 text-[12px] text-slate-700 outline-none placeholder:text-slate-400 focus:border-cyan-500 dark:border-neutral-700 dark:bg-neutral-900/80 dark:text-slate-100"
              />
              {quickInsertKeyword.length > 0 && (
                <button
                  type="button"
                  onClick={() => setQuickInsertKeyword('')}
          ...
          <div className="max-h-64 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-neutral-700">
            {quickInsertCategoryEntries.length === 0 ? (
              <div className="mt-1 rounded-md border border-dashed border-slate-300 px-3 py-4 text-center text-[12px] text-slate-500 dark:border-neutral-700 dark:text-slate-400">
                没有匹配的节点
              </div>
            ) : (
              <div className="space-y-1">
                {quickInsertCategoryEntries.map(({ category, items }) => (
                  <section key={category} className="space-y-1">
                    <div className="flex h-7 items-center gap-1.5 px-2">
                      ...
                      <span className="rounded-full bg-slate-200 px-1.5 py-0.5 text-[10px] text-slate-500 dark:bg-neutral-700 dark:text-slate-400">
                        {items.length}
                      </span>
                    </div>

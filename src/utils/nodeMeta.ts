...
- Write a small plan and finally summarize your next action (with its target element) in one sentence in \`Thought\` part.`,
  ...
  ocrMatch: {
    label: 'OCR 文字匹配',
    description: '在截图或指定源图中识别文本，并匹配目标文字。',
    defaultParams: {
      sourcePath: '',
      targetText: '',
      matchMode: 'contains',
    ...
    fields: [
      ...
      { key: 'caseSensitive', label: '区分大小写', type: 'boolean' },
      { key: 'useRegex', label: '将目标文本当作正则表达式', type: 'boolean' },
      {
        key: 'minConfidence',
        label: '最小置信度(0~1)',
        type: 'number',
      ...
      { key: 'timeoutMs', label: '超时(ms)', type: 'number', min: 0, step: 100 },
      {
        ...
        min: 1,
        step: 1,
        description: '建议 80~200ms；越小越实时，但 CPU 占用更高。',
      },
      {
        key: 'confirmFrames',
        label: '确认帧数',
